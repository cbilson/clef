...
class Track:
    def __init__(self, id, name=None, type=None, album_id=None,
                 ...
                 popularity=None, preview_url=None):
        self.id = id
        self.name = name
        self.type = type
    ...
    def _from_row(row):
        return Track(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8])
    ...
    def for_playlist(playlist):
        ...
        cursor.execute('select t.id, t.name, t.type, '
                       ...
                       't.explicit, t.popularity, t.preview_url '
                       'from Track t '
    ...
    def save(self):
        ...
        cursor.execute('insert into Track(id, name, type, '
                       ...
                       'explicit=%s, popularity=%s, preview_url=%s',
                       (self.id, self.name, self.type,
                        self.album_id, self.disc_number, self.duration_ms,
                        self.explicit, self.popularity, self.preview_url,
                        self.name, self.type,
                        self.album_id, self.disc_number, self.duration_ms,
                        self.explicit, self.popularity, self.preview_url))
